...
```
$ git clone git@github.com:wazix11/AlveusClips.git
$ cd AlveusClips
```
...
```
mysql> CREATE DATABASE alveusclips CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
mysql> CREATE USER 'alveusclips'@'localhost' IDENTIFIED BY '<db-password>';
mysql> GRANT ALL PRIVILEGES ON alveusclips.* TO 'alveusclips'@'localhost';
...
Create and fill a Supervisor configuration file for AlveusClips:
```
$ sudo nano /etc/supervisor/conf.d/alveusclips.conf

[program:alveusclips]
command=/home/ubuntu/AlveusClips/venv/bin/gunicorn -b localhost:8000 -w 4 alveusclips:app
directory=/home/ubuntu/AlveusClips
user=ubuntu
autostart=true
autorestart=true
stopasgroup=true
...
[program:scheduler]
command=/home/ubuntu/AlveusClips/venv/bin/python -m app.scheduler.scheduler
directory=/home/ubuntu/AlveusClips
autostart=true
autorestart=true
...
Set up a self-signed SSL certificate (for test deployments only), this will be created while in the AlveusClips directory:
```
$ mkdir certs
$ openssl req -new -newkey rsa:4096 -days 365 -nodes -x509 -keyout certs/key.pem -out certs/cert.pem
```
Create the Nginx configuration file for AlveusClips:
```
$ sudo nano /etc/nginx/sites-available/alveusclips
...
server {
    ...
    ssl_certificate /home/ubuntu/AlveusClips/certs/cert.pem;
    ssl_certificate_key /home/ubuntu/AlveusClips/certs/key.pem;

    # write access and error logs to /var/log
    access_log /var/log/alveusclips_access.log;
    error_log /var/log/alveusclips_error.log;
    ...
    location /static {
        ...
        alias /home/ubuntu/AlveusClips/app/static;
        expires 30d;
    }
}
```
...
```
Create a link to the AlveusClips configuration:
```
$ sudo ln -s /etc/nginx/sites-available/alveusclips /etc/nginx/sites-enabled/alveusclips
```
Update static folder permissions:
```
$ sudo find /home/ubuntu/AlveusClips/app/static -type d -exec chmod 755 {}
$ sudo find /home/ubuntu/AlveusClips/app/static -type f -exec chmod 644 {}
$ sudo chmod 755 /home
$ sudo chmod 755 /home/ubuntu
$ sudo chmod 755 /home/ubuntu/AlveusClips
$ sudo chmod 755 /home/ubuntu/AlveusClips/app
$ sudo chown -R ubuntu:www-data /home/ubuntu/AlveusClips/app/static
```
